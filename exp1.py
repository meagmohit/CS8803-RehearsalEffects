...
def main(_):
  ...
  r_index = random.randint(1,inp_labels.shape[0]-1)
  learn_word3 = np.reshape(fe[inp_words[r_index],:],[1,config.hidden_size_lstm])
  learn_label3 = np.reshape(inp_labels[r_index,:],[1,config.step_size])
  feed_dict3 = {m.init_state: learn_word3, m.input_targets: learn_label3}

  r_index = random.randint(1,inp_labels.shape[0]-1)
  learn_word4 = np.reshape(fe[inp_words[r_index],:],[1,config.hidden_size_lstm])
  learn_label4 = np.reshape(inp_labels[r_index,:],[1,config.step_size])
  feed_dict4 = {m.init_state: learn_word4, m.input_targets: learn_label4}

  r_index = random.randint(1,inp_labels.shape[0]-1)
  learn_word5 = np.reshape(fe[inp_words[r_index],:],[1,config.hidden_size_lstm])
  learn_label5 = np.reshape(inp_labels[r_index,:],[1,config.step_size])
  feed_dict5 = {m.init_state: learn_word5, m.input_targets: learn_label5}
  ...
  for i in range(1):
    ...
    while test_accuracy != 1:
      ...
      test_summary, test_accuracy, te_final_result = sess.run([merged, m.evaluation_step, m.final_result], feed_dict=feed_dict2)
      test_writer.add_summary(test_summary, t)
    iter_word2=t
    t=0
    test_accuracy = 0
    while test_accuracy != 1:
      t=t+1
      train_summary, _ = sess.run([merged, m.train_step], feed_dict=feed_dict3)
      train_writer.add_summary(train_summary, t)
      train_accuracy, loss_value, tr_final_result = sess.run([m.evaluation_step, m.loss_mean, m.final_result], feed_dict=feed_dict3)
      test_summary, test_accuracy, te_final_result = sess.run([merged, m.evaluation_step, m.final_result], feed_dict=feed_dict3)
      test_writer.add_summary(test_summary, t)
    iter_word3=t
    t=0
    test_accuracy = 0
    while test_accuracy != 1:
      t=t+1
      train_summary, _ = sess.run([merged, m.train_step], feed_dict=feed_dict4)
      train_writer.add_summary(train_summary, t)
      train_accuracy, loss_value, tr_final_result = sess.run([m.evaluation_step, m.loss_mean, m.final_result], feed_dict=feed_dict4)
      test_summary, test_accuracy, te_final_result = sess.run([merged, m.evaluation_step, m.final_result], feed_dict=feed_dict4)
      test_writer.add_summary(test_summary, t)
    iter_word4=t
    t=0
    test_accuracy = 0
    while test_accuracy != 1:
      t=t+1
      train_summary, _ = sess.run([merged, m.train_step], feed_dict=feed_dict5)
      train_writer.add_summary(train_summary, t)
      train_accuracy, loss_value, tr_final_result = sess.run([m.evaluation_step, m.loss_mean, m.final_result], feed_dict=feed_dict5)
      test_summary, test_accuracy, te_final_result = sess.run([merged, m.evaluation_step, m.final_result], feed_dict=feed_dict5)
      test_writer.add_summary(test_summary, t)
    iter_word5=t
  
  print("Final_Result: # of iterations required for word1 and word2")
  print(iter_word1)
  print(iter_word2)
  print(iter_word3)
  print(iter_word5)
  f = open('results/results_exp1.csv', 'a')
  writer = csv.writer(f,quoting=csv.QUOTE_MINIMAL)
  writer.writerow([iter_word1, iter_word2, iter_word3, iter_word5])
