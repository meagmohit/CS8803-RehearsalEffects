...
tf.app.flags.DEFINE_string('mode','',""" possible: train, finetune, evaluation, livefeed, sgtrain, sgfinetune""")

tf.app.flags.DEFINE_integer('iterations',200, """ Total number of training iterations for a particular word(not epochs) """)
...
class model(object):
  ...
  def __init__(self, is_training, config):
    ...
    with tf.name_scope(layer_name):
      ...
      embedding = variable_scope.get_variable("embedding",[num_class, 128]) 
...
def main(_):
  ...
  #input_stream = input.input(FLAGS.data_path, config, image_data_tensor=jpeg_data_tensor, bottleneck_tensor=bottleneck_tensor) #TD1
  ...
  with open('word2vec_var.p') as f:
    count,d,rd,fe = pickle.load(f)
  wordinput = np.reshape(fe[41628,:],[1,128])
  target = np.array([18,9,3,8,12,1,14,4,27])
  words_input = []
  for i in xrange(50000):
    if len(rd[i]) == (config.step_size-1):
      words_input.append(d[rd[i]])
  learn_word = np.reshape(fe[words_input[0],:],[1,128]) # The word on which memory is tested
  input_labels = []
  for j in xrange(len(words_input)):
    s = rd[words_input[j]]
    x = [ord(i)-96 for i in list(s)]
    x.append(27)
    input_labels.append(x)
  words_input = np.asarray(words_input)
  input_labels = np.asarray(input_labels)
  print(input_labels.shape)
  learn_label = np.reshape(input_labels[0,:],[1,9])
  ...
  for pp in range(5):
    for i in range(FLAGS.iterations):
      t=t+1
      feed_dict = {m.init_state: learn_word, m.input_targets: learn_label}
      train_summary, _ = sess.run([merged, m.train_step], feed_dict=feed_dict)
      train_writer.add_summary(train_summary, t)
      train_accuracy, loss_value, tr_final_result = sess.run([m.evaluation_step, m.loss_mean, m.final_result], feed_dict=feed_dict)
      test_summary, test_accuracy, te_final_result = sess.run([merged, m.evaluation_step, m.final_result], feed_dict=feed_dict)
      test_writer.add_summary(test_summary, t)
      print("Iteration:  "+str(t))
      print(te_final_result)
      print(test_accuracy)

    for k in range(10):
      r_index = random.randint(1,input_labels.shape[0])
      learn_words = np.reshape(fe[words_input[r_index],:],[1,128])
      learn_labels = np.reshape(input_labels[r_index,:],[1,9])
      for i in range(FLAGS.iterations):
        t=t+1
        feed_dict2 = {m.init_state: learn_words, m.input_targets: learn_labels}
