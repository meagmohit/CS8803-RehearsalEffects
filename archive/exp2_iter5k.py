...
tf.app.flags.DEFINE_integer('iterations',5000, """ Total number of training iterations for a particular word(not epochs) """)
...
def main(_):
  ...
  with open('dataset_size16.p') as f:
    inp_words, inp_labels, fe = pickle.load(f)
  
  learn_words = []
  learn_labels = []
  final_result = []
  print_list = [0,1,4,9,19,49,99]
  t=0
  for i in range(100):
    #print(i)
    r_index = random.randint(1,inp_labels.shape[0])
    learn_word = np.reshape(fe[inp_words[r_index],:],[1,config.hidden_size_lstm])
    learn_label = np.reshape(inp_labels[r_index,:],[1,config.step_size])
    learn_words.append(learn_word)
    learn_labels.append(learn_label)
    feed_dict = {m.init_state: learn_words[i], m.input_targets: learn_labels[i]}
    ...
    if i in print_list:
      for j in range(i+1):
        feed_dict = {m.init_state: learn_words[j], m.input_targets: learn_labels[j]}
      ...
      final_result.append(acc/(i+1))
      
  f = open('results_exp2_iter5k.csv', 'a')
